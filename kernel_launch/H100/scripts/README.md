...
| 脚本 | 作用 |
|------|------|
| `config.py` | 集中定义模型 `MODELS`（Qwen3 4 稠密 + Qwen3-30B-A3B + Qwen3.5-0.8B/2B/9B/27B）、执行模式、Attention 后端、`CASES`(含 `batch_size`/`prompt_len`/`decode_len` 的 13 个用例)、结果目录，以及组合遍历 `combos()` / 命名 `run_tag()`。 |
