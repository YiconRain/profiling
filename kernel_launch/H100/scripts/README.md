...
| 脚本 | 作用 |
|------|------|
| `config.py` | 集中定义模型 `MODELS`（Qwen3 4 稠密 + Qwen3-30B-A3B）、执行模式、Attention 后端、`CASES`(含 `batch_size`/`prompt_len`/`decode_len` 的 13 个用例)、结果目录，以及组合遍历 `combos()` / 命名 `run_tag()`。 |
...
| `analyze_nsys.py` | 离线分析。采集范围已限定在被测 generate,直接统计库内全部：**gpu_busy(区间并集)、gpu_bubble_ratio(核心)**、kernel 数、launch 类 API 次数/开销、launch 占比(诊断)、Top-5 kernel;e2e 取主机 API 时间跨度。 |
...
| `run_all.sh` | 一键驱动：`download_models.py --series qwen3` 下载 Qwen3 系列 → 跑采集 → 汇总。环境变量 `SGL_PY` 可指定 SGLang 解释器（默认 `~/envs/sgl_env/bin/python`）。 |
