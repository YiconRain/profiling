...
<center> BS=1、prompt=16、decode=512 的 Decode-only GPU Bubble Ratio</center>
...
不过，在小模型的 prefill 和 decode（实际应用价值有限）、大模型短 prompt 下 prefill（prompt < 1k，稍微真实一些的场景）、MOE（真实）下，即使使用 CUDA Graph，我们往往依然观察到 10% 级别的 GPU bubble，如果通过 MegaKernel 架构能充分利用这部分 bubble，同时保证 GPU 调度/同步逻辑不反过来拖累 GPU kernel 的运行，MegaKernel 的这个 `通过减少 kernel 数量降低 launch/host overhead` 就依然是有意义的。

补充观察 Qwen3-30B-A3B 这个 MoE 模型在 cudagraph 模式下的 case-level GPU bubble：

| Workload     | Case          |  BS | Prompt | Decode | GPU bubble ratio | GPU bubble ms |
| ------------ | ------------- | --: | -----: | -----: | ---------------: | ------------: |
